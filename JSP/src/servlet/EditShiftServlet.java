		 ...
		 HttpSession session = request.getSession(false);
			if(session==null || !request.isRequestedSessionIdValid() )
			{
				...
				if(session.getAttribute("userinfo")==null){
					...
					try {
						training=db.gettraining(((ArrayList<ShiftInfo>) session.getAttribute("schedulesearch")).get(Integer.parseInt(session.getAttribute("shiftnumber").toString())).getShift().getShift());
						//training =db.gettraining((ArrayList<ShiftInfo>)session.getAttribute("schedulesearch")).get(Integer.parseInt(session.getAttribute("shiftnumber").toString()));
					} catch (Exception e) {
					...
					//session.setAttribute("training", db.gettraining(request.getParameter("upvote"));
